...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask, image_name, uid,
                 ...
                 rgb_image=None, physical_image=None, is_rgbt=False):
        ...
        self.original_rgb_image = rgb_image.clamp(0.0, 1.0).to(self.data_device) if rgb_image is not None else None
        self.original_physical_image = physical_image.clamp(0.0, 1.0).to(self.data_device) if physical_image is not None else None
        self.is_rgbt = bool(is_rgbt and self.original_rgb_image is not None)
        ...
        if gt_alpha_mask is not None:
            ...
            if self.original_rgb_image is not None:
                self.original_rgb_image *= gt_alpha_mask.to(self.data_device)
            if self.original_physical_image is not None:
                self.original_physical_image *= gt_alpha_mask.to(self.data_device)
        else:
            self.original_image *= torch.ones((1, self.image_height, self.image_width), device=self.data_device)
            if self.original_rgb_image is not None:
                self.original_rgb_image *= torch.ones((1, self.image_height, self.image_width), device=self.data_device)
            if self.original_physical_image is not None:
                self.original_physical_image *= torch.ones((1, self.image_height, self.image_width), device=self.data_device)
