...
class GeoRefineNet(nn.Module):
    def __init__(self, device, D=8, W=256, input_ch=3, output_ch=59, multires=10, is_blender=False, is_6dof=False):
        ...
        self.input_ch_base = xyz_input_ch + time_input_ch + 3 + 3

        self.input_ch = self.input_ch_base


        self.linear = nn.ModuleList(
            [nn.Linear(self.input_ch, self.W).to(device)] + [
                nn.Linear(self.W, self.W).to(device) if i not in self.skips else nn.Linear(self.W + self.input_ch, self.W).to(device)
    ...
    def forward(self, x, t, cam_pos=None, view_dir=None):
        ...
        h = torch.cat([x_emb, t_emb, cam_pos, view_dir], dim=-1)

        for i, l in enumerate(self.linear):
            h = l(h)
            h = F.relu(h)
            if i in self.skips:
                h = torch.cat([x_emb, t_emb, cam_pos, view_dir, h], -1)

        # self.h_values.append(h.detach().cpu().numpy())  # Store h values for MI calculation
